...
export const VideoViewComponent: React.FC<DefaultComponentProps> = (props: DefaultComponentProps) => {
    //create webcam ref
    const webcamRef = useRef<Webcam>(null);
    //define video view action button properties
    const [actionButtonState, setActionButtonState] = useState<boolean>(false);
    const [videoViewActionButtonProps, setVideoViewActionButtonProps] = useState<string[]>(["Streaming Starten", "Icon_Play_IOS_Primary_Color"]);
    ...
    return (
        <>
            <div id={props.id} className={`video-view-style-properties ${props.className}`} style={props.style}>

                loading
            </div>
            <div style={{marginTop: '-1.5%'}}>
                <ImageLink textValue={videoViewActionButtonProps[0]} textImage={`images/general/${videoViewActionButtonProps[1]}.png`} textLinkAction={videoStreamAction}/>
            </div>
        </>
    );
}