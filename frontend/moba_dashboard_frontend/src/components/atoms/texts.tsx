...
const Text: React.FC<TextProperties> = (props: TextProperties) => {
    return (
        <span id={props.textID} style={props.style} className={props.className}>{props.textValue}</span>
    );
}
...
export const HeaderText: React.FC<TextProperties> = (props: TextProperties) => {
    return (
        <Text textID={props.textID} style={props.style} className="header-text-component-style-properties" textValue={props.textValue}/>
    );
}
...
export const TextLink: React.FC<TextProperties> = (props: TextProperties) => {
    return (
        <a id={props.textID} style={props.style} className={props.className}>{props.className}</a>
    );
}
...
export const ImageLink: React.FC<TextProperties> = (props: TextProperties) => {
    ...
    function executeOnClickEvent() {
        ...
        if (props.textLinkUrl) {
            window.open((!props.textValue.includes('https://')) ? `https://${props.textLinkUrl}` : props.textLinkUrl, "_blank")
        }
    }
    ...
    return (
        <FlexBox id={props.textID} clickAction={executeOnClickEvent}>
            <Image style={{height: '18px', width: 'auto', marginRight: '2%'}} imageValue={props.textImage}/>
            <TextLink style={props.style} className={props.className} textValue={props.textValue}/>
        </FlexBox>
    );
}