...
export const SelectOptionItem: React.FC<SelectOptionItemProps> = (
	props: SelectOptionItemProps,
) => {
	return (
		<option
			id={props.id}
			style={props.style}
			value={props.selectOptionItemValue}
			className={props.className}
		>
			{props.selectOptionItemText}
		</option>
	);
};
	...
	return (
		...
		>
			<input
				id={props.id}
				type="checkbox"
				onChange={props.checkBoxAction}
				checked={props.checkBoxDefaultCheckState}
			/>
			<Text
				style={{ fontSize: "18px", marginLeft: "3%" }}
				textValue={props.checkBoxText!}
			/>
		</FlexBox>
	);
};
