...
public class CANMessage {
    ...
    /**
     * the first 4 bits of a frame are the priority
     *
    ...
    /**
     * Evaluates the command of a given frame. According to documentation, the command takes up 8 bits in a frame
     * from the last bit of the first byte to the 7th bit of the second byte. However, the first bit is always zero, so it's not necessary to consider the first byte
     */
    ...
    /**
     * the last bit of the second byte is the response bit
     *
     ...
     * They are the last 4 bits of the 5th byte.
     *
    ...
    /**
     * the last 8 bytes of a frame contain the
     * data bytes
     *
